...
from evaluation.jet_matching import calculate_pur_eff_vs_variable


def plot_signal_background_histogram(
    reco_jets, is_pure_mask, bins, variable, xlabel, title
):
    """
    ...
    background_data = ak.to_numpy(
        ak.flatten(getattr(reco_jets[~is_pure_mask], variable))
    )
    ...
    plt.hist(
        background_data, bins=bins, histtype="step", label="Background", color="red"
    )
...
def plot_roc_comparison(roc_results, working_point=None, title="", return_fig=False):
    """
    ...
    if working_point != None:
        ax.vlines(
            ...
            colors="black",
            linestyles="dashed",
            label=f"WP = {working_point*100}% Mistag rate",
        )
...
def plot_btag_map(jets, tagger_name, pt_bins, eta_bins):
    ...
    h_count_jets, _, _ = np.histogram2d(jet_eta, jet_pt, bins=[eta_bins, pt_bins])
...
def plot_attr_vs_var_proj(
    ...
    gs = gridspec.GridSpec(
        2, 2, height_ratios=[3, 1], hspace=0.05, width_ratios=[3, 1], wspace=0.05
    )
    ...
    ax0.set_yticks(
        np.linspace(min(bins_attr), max(bins_attr), 11)
    )  # Reduce number of y-ticks for clarity
    ax0.set_xticks(
        np.linspace(min(bins_var), max(bins_var), 11)
    ...
    ax2.set_xticks(
        np.linspace(0, max(attr_counts), 2)
    )  # Reduce number of x-ticks for clarity
    ax2.set_yticks(
        np.linspace(min(bins_attr), max(bins_attr), 11)
    )  # Reduce number of y-ticks for clarity
...
def plot_avg_attr_vs_var(
    reco_jets, is_signal_mask, attr, variable, bins, xlabel, ylabel, title
):
    """
    Plots the average of a given attribute vs. a variable for signal and background jets.
    """
    attr_signal_flattened = ak.to_numpy(
        ak.flatten(getattr(reco_jets[is_signal_mask], attr))
    )
    variable_signal_flattened = ak.to_numpy(
        ak.flatten(getattr(reco_jets[is_signal_mask], variable))
    )
    attr_background_flattened = ak.to_numpy(
        ak.flatten(getattr(reco_jets[~is_signal_mask], attr))
    )
    variable_background_flattened = ak.to_numpy(
        ak.flatten(getattr(reco_jets[~is_signal_mask], variable))
    )
    ...
    for i in range(len(bins) - 1):
        bin_mask_signal = (variable_signal_flattened >= bins[i]) & (
            variable_signal_flattened < bins[i + 1]
        )
        bin_mask_background = (variable_background_flattened >= bins[i]) & (
            variable_background_flattened < bins[i + 1]
        )
        if np.sum(bin_mask_signal) > 0:
            avg_signal_attr_per_var_bin.append(
                np.mean(attr_signal_flattened[bin_mask_signal])
            )
            std_signal_attr_per_var_bin.append(
                np.std(attr_signal_flattened[bin_mask_signal])
                / np.sqrt(np.sum(bin_mask_signal))
            )
        else:
            avg_signal_attr_per_var_bin.append(0)
            std_signal_attr_per_var_bin.append(0)
        if np.sum(bin_mask_background) > 0:
            avg_background_attr_per_var_bin.append(
                np.mean(attr_background_flattened[bin_mask_background])
            )
            std_background_attr_per_var_bin.append(
                np.std(attr_background_flattened[bin_mask_background])
                / np.sqrt(np.sum(bin_mask_background))
            )
        else:
    ...
    plt.errorbar(
        bin_centers,
        avg_signal_attr_per_var_bin,
        yerr=std_signal_attr_per_var_bin,
        marker="o",
        label="Signal Avg",
    )
    plt.errorbar(
        bin_centers,
        avg_background_attr_per_var_bin,
        yerr=std_background_attr_per_var_bin,
        marker="o",
        label="Background Avg",
    )
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
...
def plot_resolution_vs_var(gen_var, resolution_values, bins, y_label, x_label, title):
    ...
    plt.subplot(2, 1, 2)
    plt.errorbar(
        bin_centers, means, fmt="s--", color="red", capsize=5, label="Scale (Mean)"
    )
